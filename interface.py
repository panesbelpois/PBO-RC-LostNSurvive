...
def draw_inventory_panel(screen, font, wagon, screen_width, screen_height):
    panel_width = 500
    panel_height = 350
    panel_x = (screen_width - panel_width) // 2
    panel_y = (screen_height - panel_height) // 2

    pygame.draw.rect(screen, (100, 50, 20), (panel_x, panel_y, panel_width, panel_height), border_radius=12)
    pygame.draw.rect(screen, (180, 130, 80), (panel_x, panel_y, panel_width, panel_height), 3, border_radius=12)

    draw_text(screen, font, "Inventaris", panel_x + panel_width // 2, panel_y + 20, (255, 255, 255), center_x=True)
    ...
    y_offset = panel_y + 60
    for kategori, item_list in wagon.inventory.items():
        if kategori == "uang":
            draw_text(screen, font, f"Uang: {item_list}", panel_x + 20, y_offset, (255, 255, 100))
            y_offset += 30
            continue
        draw_text(screen, font, f"{kategori.capitalize()}:", panel_x + 20, y_offset, (255, 200, 150))
        y_offset += 30
        for item, restore in item_list:
            draw_text(screen, font, f"- {item} (+{restore} {kategori_map.get(kategori, '')})", panel_x + 40, y_offset)
            y_offset += 25

    draw_text(screen, font, "Tekan [2] untuk menutup", panel_x + panel_width // 2,
              panel_y + panel_height - 30, (200, 200, 200), center_x=True)