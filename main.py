...
story_backgrounds = [pygame.image.load("BACKGROUND_OPENING_TRY.png").convert()]
...
def start_battle(crew_list):
    ...
    while running:
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if battle.crew_selection_phase:
                    ...
                    if len(battle.player_crew) > 0:  # Fixed: player_crew is a property of battle, not self
                        start_button = pygame.Rect(350, 550, 200, 50)
                        if start_button.collidepoint(mouse_pos):
                            battle.start_battle()
                else:
                    # Modified to use crew positions correctly
            ...
            if event.type == pygame.KEYDOWN:
                ...
                # Add support for individual key presses for movement
                if not battle.crew_selection_phase and battle.selected_crew:
                    ...
                    # Keep crew within bounds (moved here from handle_movement_input for single key presses)
        ...
        # Handle continuous movement with key presses - FIX: Only call if selected_crew is not None
        keys = pygame.key.get_pressed()
        if battle.selected_crew is not None:
            battle.handle_movement_input(keys)

        # Update battle state (new in this version)
        battle.update()
    ...
    # Sinkronisasi HP ke objek Crew
    for c in crew_list:
        for b in battle.player_crew:
            if c.nama == b.nama:
                c.hp = b.hp
                c.is_dead = b.is_dead  # Make sure is_dead status gets synced too
        ...
        if show_input_box:
            ...
            if input_result:
                if len(my_wagon.crew_list) < my_wagon.capacity:
                    my_wagon.crew_list.append(Crew(input_result, 50, 50, random.randint(5, 10), 100))
