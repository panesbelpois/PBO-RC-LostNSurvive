...
class Event:
    def __init__(self):
        ...
        self.events_cuaca = [
            {"deskripsi": "Mau ngapain kack",
             "pilihan": ["GTW SUMPAH", "Pencet ini lgsg game over :V"],
    ...
    def apply_consequence(self, event, pilihan, wagon):
        ...
        if tipe == "musuh":
            ...
            if pilihan == "Terima":
                if hasattr(wagon, "inventory"):
                    wagon.inventory.setdefault("food", []).append(("Ham", 10))
                    wagon.inventory.setdefault("drink", []).append(("Air", 8))
                msg += " (+Ham & Air)"
            elif pilihan == "Menjelajah":
                if random.random() < 0.7:
                    if hasattr(wagon, "inventory"):
                        wagon.inventory.setdefault("food", []).append(("Sausage", 12))
                    msg += " (+Sausage)"
                else:
                    if wagon.crew_list:
                        crew = random.choice(wagon.crew_list)
